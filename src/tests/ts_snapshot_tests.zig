...
test "TypeScript snapshot contains stable tab identities and bounded titles" {
    ...
    const snapshot = try app.encodeTsSnapshot(&model, 7, 11, &bytes);
    ...
    const first_id = std.mem.readInt(u32, snapshot[24..28], .little);
    const first_title_len: usize = snapshot[29];
    const second_at = 30 + first_title_len;
